...
generate_systemd_conf() {
    ...
    local -r service_file="${systemd_units_dir}/drycc.${service_name}.service"
    # Default values
...
[Unit]
Description=Drycc service for ${name}
# Starting/stopping the main drycc service should cause the same effect for this service
PartOf=drycc.service
...
[Install]
# Enabling/disabling the main drycc service should cause the same effect for this service
WantedBy=drycc.service
EOF
}
