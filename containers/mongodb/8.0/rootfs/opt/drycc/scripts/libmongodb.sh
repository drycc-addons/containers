...
mongodb_set_journal_conf() {
    ...
    if ! mongodb_is_file_external "$conf_file_name"; then
        # Disable journal.enabled since it is not supported from 7.0 on (including 8.0)
        if [[ "$(mongodb_get_major_version)" -gt 7 ]]; then
            # For MongoDB 7.0+ (including 8.0), remove journal configuration
            mongodb_conf="$(sed '/journal:/,/enabled: .*/d' "$conf_file_path")"
            echo "$mongodb_conf" >"$conf_file_path"
        else
            # For MongoDB versions < 7.0, use journal configuration if specified
