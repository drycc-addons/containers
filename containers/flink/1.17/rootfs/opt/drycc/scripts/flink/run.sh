...
if [[ "$FLINK_MODE" = "help" ]]; then
    ...
    printf "By default, the Apache Flink Packaged by Drycc image will run in jobmanager mode.\n"
    printf "Also, by default, Apache Flink Packaged by Drycc image adopts jemalloc as default memory allocator. This behavior can be disabled by setting the 'DISABLE_JEMALLOC' environment variable to 'true'.\n"
    exit 0
