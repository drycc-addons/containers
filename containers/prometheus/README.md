...
```console
docker run --name prometheus drycc/prometheus:latest
```
...
* All our images are based on [minideb](https://github.com/drycc/minideb) a minimalist Debian based container image which gives you a small base container image and the familiarity of a leading Linux distribution.
* All Bitnami images available in Docker Hub are signed with [Docker Content Trust (DCT)](https://docs.docker.com/engine/security/trust/content_trust/). You can use `DOCKER_CONTENT_TRUST=1` to verify the integrity of the images.
* Bitnami container images are released on a regular basis with the latest distribution packages available.

Looking to use Prometheus in production? Try [VMware Application Catalog](https://drycc.com/enterprise), the enterprise edition of Bitnami Application Catalog.

## Why use a non-root container?

Non-root container images add an extra layer of security and are generally recommended for production environments. However, because they run as a non-root user, privileged tasks are typically off-limits. Learn more about non-root containers [in our docs](https://docs.drycc.com/tutorials/work-with-non-root-containers/).

## Supported tags and respective `Dockerfile` links

Learn more about the Bitnami tagging policy and the difference between rolling tags and immutable tags [in our documentation page](https://docs.drycc.com/tutorials/understand-rolling-tags-containers/).
...
The recommended way to get the Bitnami Prometheus Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/drycc/prometheus).

```console
docker pull drycc/prometheus:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/drycc/prometheus/tags/) in the Docker Hub Registry.

```console
docker pull drycc/prometheus:[TAG]
```
...
For persistence, mount a directory at the `/opt/drycc/prometheus/data` path. If the mounted directory is empty, it will be initialized on the first run.

```console
docker run --name prometheus \
    -v /path/to/prometheus-persistence:/opt/drycc/prometheus/data \
    drycc/prometheus:latest
```
...
```console
docker run --name prometheus-node1 --network prometheus-network drycc/prometheus:latest
```
...
To view all available command-line flags, run `docker run drycc/prometheus:latest -h`.

### Configuration file

You can overwrite the default configuration file with your custom `prometheus.yml`. Create a custom conf file and mount it at `/opt/drycc/prometheus/conf/prometheus.yml` like so:

```console
docker run --name prometheus \
    -v path/to/prometheus.yml:/opt/drycc/prometheus/conf/prometheus.yml \
    drycc/prometheus:latest
```
