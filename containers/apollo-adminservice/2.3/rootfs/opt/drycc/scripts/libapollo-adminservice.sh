...
apollo_conf_get() {
    ...
    local -r conf_file="${2:-"${APOLLO_BASE_DIR}/apollo-adminservice.conf"}"
...
apollo_conf_set() {
    ...
    replace_in_file "${APOLLO_BASE_DIR}/apollo-adminservice.conf" "^#*\s*${key}.*" "${key}=${value}" false
}
...
#########################
apollo_configure_default() {
    info "Initializing Apollo Adminservice"
