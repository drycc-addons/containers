...
apollo_conf_get() {
    ...
    local -r conf_file="${2:-"${APOLLO_BASE_DIR}/apollo-portal.conf"}"
...
apollo_conf_set() {
    ...
    replace_in_file "${APOLLO_BASE_DIR}/apollo-portal.conf" "^#*\s*${key}.*" "${key}=${value}" false
}
...
#########################
apollo_configure_default() {
    info "Initializing Apollo portal"
