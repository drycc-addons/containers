...
#!/bin/bash
#
# Drycc Elasticsearch/Opensearch common library
...
elasticsearch_validate() {
    ...
    validate_node_roles() {
        if [ -n "$DB_NODE_ROLES" ]; then
            ...
            [[ "$DB_FLAVOR" = "opensearch" && "$OPENSEARCH_VERSION" =~ ^2\. ]] && master_role="cluster_manager"
...
get_elasticsearch_roles() {
    ...
    for i in "${!roles_list[@]}"; do
        if [[ ${roles_list[$i]} == "master" ]] && [[ "$DB_FLAVOR" = "opensearch" && "$OPENSEARCH_VERSION" =~ ^2\. ]]; then
...
elasticsearch_cluster_configuration() {
    ...
    if [[ -n "$DB_CLUSTER_HOSTS" ]]; then
        ...
        if is_node_master; then
            if [[ "$DB_FLAVOR" = "opensearch" && "$OPENSEARCH_VERSION" =~ ^2\. ]]; then
