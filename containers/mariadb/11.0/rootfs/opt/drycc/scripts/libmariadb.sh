...
!include ${DB_CONF_DIR}/drycc/my_custom.cnf
EOF
}
...
mysql_start_bg() {
    ...
    debug_execute "${DB_BIN_DIR}/mysqld" "${flags[@]}" &
...
#########################
mysql_install_db() {
    local command="${DB_SCRIPTS_DIR}/mysql_install_db"
...
#########################
mysql_healthcheck() {
    local args=("--defaults-file=$DB_CONF_FILE" "-uroot" )
    local root_password
