...
. /opt/drycc/scripts/libversion.sh
...
EOF
        ...
        mysql_execute "mysql" "$DB_ROOT_USER" "$DB_ROOT_PASSWORD" <<EOF
create user if not exists '$user'@'%' $([ "$password" != "" ] && echo "identified with 'mysql_native_password' by \"$password\"");
EOF
    fi
    mysql_execute "mysql" "$DB_ROOT_USER" "$DB_ROOT_PASSWORD" <<EOF
...
mysql_initialize() {
    ...
    if [[ -f "${DB_CONF_DIR}/my_custom.cnf" ]]; then
        if is_file_writable "${DB_CONF_DIR}/drycc/my_custom.cnf"; then
            info "Injecting custom configuration 'my_custom.cnf'"
            cat "${DB_CONF_DIR}/my_custom.cnf" > "${DB_CONF_DIR}/drycc/my_custom.cnf"
            if ! grep --silent "!include ${DB_CONF_DIR}/drycc/my_custom.cnf" "${DB_CONF_FILE}"; then
                echo "!include ${DB_CONF_DIR}/drycc/my_custom.cnf" >> "${DB_CONF_FILE}"
            fi
        else
            warn "Could not inject custom configuration for the ${DB_FLAVOR} configuration file '$DB_CONF_DIR/drycc/my_custom.cnf' because it is not writable."
        fi
    fi
...
mysql_execute_print_output() {
    ...
    if [[ "${DRYCC_DEBUG:-false}" = true ]]; then
        local mysql_cmd
        mysql_cmd="$(</dev/stdin)"
        ...
        # https://github.com/drycc/drycc-docker-mariadb/issues/251
...
mysql_migrate_old_configuration() {
    ...
    local -r custom_conf_file="$DB_CONF_DIR/drycc/my_custom.cnf"
