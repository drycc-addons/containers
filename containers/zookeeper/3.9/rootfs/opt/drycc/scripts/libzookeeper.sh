...
# See https://github.com/drycc/charts/pull/1390/files#diff-6b063ad92827264b128cc05c45bd9232L85-L90
# Globals:
#   None
# Arguments:
#   None
...
zookeeper_ensure_backwards_compatibility() {
    mkdir -p "/opt/drycc/base"
    cat >"/opt/drycc/base/functions" <<EOF
#!/bin/bash

# Load Generic Libraries
. /opt/drycc/scripts/liblog.sh

warn "You are probably using an old version of the drycc/zookeeper Helm Chart. Please consider upgrading to 5.0.0 or later."

exec /opt/drycc/scripts/entrypoint.sh /opt/drycc/scripts/run.sh
EOF
}
